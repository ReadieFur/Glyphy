...
namespace Glyphy.LED
{
    ...
    {
        private Dictionary<EAddressable, float> brightnessValues = new();
        ...
        public override Task<float> GetBrightness(EGroup ledGroup)
        {
            List<float> addressableLEDValues = new();
            ...
            foreach (float brightness in addressableLEDValues)
                averageBrightness += brightness;
            averageBrightness /= addressableLEDValues.Count;

            return Task.FromResult(averageBrightness);
        }

        public override Task<float> GetBrightness(EAddressable addressableLED) =>
            Task.FromResult(brightnessValues[addressableLED]);

        public override Task SetBrightness(EGroup ledGroup, float brightness)
        {
        ...
        public override Task SetBrightness(EAddressable addressableLED, float brightness)
        {
            brightnessValues[addressableLED] = brightness;
            return Task.CompletedTask;
        }
